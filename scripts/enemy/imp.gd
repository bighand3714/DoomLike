...
# Imp — 小恶魔
# ==============================================================================
# 混合攻击：远程火球(>2m) / 近战爪击(≤2m)。
# 三段式攻击：PREPARE(蓄力发光)→ACTIVE(火球/爪击)→RECOVER(短暂收招)。
# 距离档位路由：FAR/MEDIUM→接近+火球 / CLOSE/MELEE→近战爪击。
# 外观：棕红色 CSG 人形 + 黄色眼睛
# ==============================================================================
...
func _state_entered(new_state: EnemyState) -> void:
	match new_state:
		EnemyState.ATTACK_PREPARE:
			_set_arms_emission(Color.ORANGE, 1.0)


func _state_exit(old_state: EnemyState) -> void:
	match old_state:
		EnemyState.ATTACK_PREPARE:
			_set_arms_emission(Color.BLACK, 0.0)
...
# 工具
# ==============================================================================
func _set_arms_emission(color: Color, energy: float) -> void:
	for child in get_children():
		if child is CSGBox3D and "name" in child:
